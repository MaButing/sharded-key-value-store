...
public:
    ...
    int comm_send(int dest_id, void* buff, size_t buff_size) const;
    int comm_recv(int* source_id, void* buff, size_t buff_size, int timeout_sec = 0) const;
    int comm_close();
    int comm_sendOut(const std::string& ip_str, int port, void* buff, size_t buff_size) const;
    int comm_sendOut(const sockaddr_in& destaddr, void* buff, size_t buff_size) const;
